...
def run_udp_multicast_listener():
    ...
    # Bind the socket interface explicitly to all available local network adapters on the multicast port
    sock.bind(('0.0.0.0', MULTICAST_PORT))
    
    # Pack the multicast IP and INADDR_ANY (0.0.0.0) into explicit 4-byte binary buffers ('4s4s')
    # This prevents struct padding errors between the internal Linux kernel and the Windows runtime host
...
def send_multicast_broadcast():
    ...
    # Force the local loopback interface driver to push the packet outward across the Docker network bridge
